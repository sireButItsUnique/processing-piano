/* autogenerated by Processing revision 1293 on 2023-11-26 */
...
public class Key { 
	...
	public boolean isMouseOver(int pos, boolean blackOnLeft, boolean blackOnRight) {
		...
		if (this.sharp) {
			float startX = keyWidth * pos + (keyWidth - (0.3f * keyWidth));
			float endX = startX + (0.6f * keyWidth);
			return (mouseX > startX && mouseX < endX && mouseY < (0.8f * keyHeight));
		} 
		...
		// check for white keys
			...
			if (mouseX > startX && mouseX < endX && mouseY < keyHeight) {
				if (blackOnLeft && mouseY < (0.8f * keyHeight)) {
					if (mouseX < startX + (0.3f * keyWidth)) {
						return false;
					}
				}
				if (blackOnRight && mouseY < (0.8f * keyHeight)) {
					if (mouseX > endX - (0.3f * keyWidth)) {
						return false;
					}
				}
				return true;
	...
	public void render(int pos, boolean blackOnLeft, boolean blackOnRight) {
		...
		// render black key
 		if (this.sharp) {
			...
			rect(startX, 0, (0.6f * keyWidth), (0.8f * keyHeight));
			if (this.isMouseOver(pos, blackOnLeft, blackOnRight)) {
				fill(255);
				textSize(25);
				text((char)('A' + this.note) + "#", startX + (0.3f * keyWidth), (0.8f * keyHeight) - 20); // char is just int mapped to ascii
			}
		} 
		...
		// render white key
			...
			if (this.isMouseOver(pos, blackOnLeft, blackOnRight)) {
				fill(0);
				textSize(40);
				text((char)('A' + this.note), startX + (0.5f * keyWidth), keyHeight - 20); // char is just int mapped to ascii
			}
		}
...
public void mouseClicked() {
  ...
  public void settings() { size(1800, 600); }
